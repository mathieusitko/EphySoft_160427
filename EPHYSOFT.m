function varargout = EPHYSOFT(varargin)
% EPHYSOFT MATLAB code for EPHYSOFT.fig
%      EPHYSOFT, by itself, creates a new EPHYSOFT or raises the existing
%      singleton*.
%
%      H = EPHYSOFT returns the handle to a new EPHYSOFT or the handle to
%      the existing singleton*.
%
%      EPHYSOFT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in EPHYSOFT.M with the given input arguments.
%
%      EPHYSOFT('Property','Value',...) creates a new EPHYSOFT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before EPHYSOFT_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to EPHYSOFT_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help EPHYSOFT

% Last Modified by GUIDE v2.5 24-Feb-2016 17:06:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @EPHYSOFT_OpeningFcn, ...
                   'gui_OutputFcn',  @EPHYSOFT_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before EPHYSOFT is made visible.
function EPHYSOFT_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to EPHYSOFT (see VARARGIN)

% Choose default command line output for EPHYSOFT
...
bgPCAGERVA = imread('cluster.png');
bgPCAGERVA = imresize(bgPCAGERVA, [position(4) position(3)]);
set(handles.PCA_Gerva,'CData',bgPCAGERVA)
...
% UIWAIT makes EPHYSOFT wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = EPHYSOFT_OutputFcn(hObject, eventdata, handles) 
