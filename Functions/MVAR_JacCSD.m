...
[~, Mp] = size(A); % MVAR coefficients matrix (Mp x Mp)
h = .00000001;
...
for i = 1 : M*Mp
    ...
    tmp_a(i) = tmp_a(i) + 1i*h; % a0 + ih
    tmp_A = reshape(tmp_a, M*p, M)';
    tmp_A_bis = [tmp_A; eye(size(tmp_A,2)-size(tmp_A,1),size(tmp_A,2)-size(tmp_A,1)) zeros(size(tmp_A,2)-size(tmp_A,1),size(tmp_A,1))];
    tmp_fa = tmp_A_bis * x_ext + randn(Mp,1); % f(a0+ih)
    J_A(:,i) = imag(tmp_fa/h);
end
