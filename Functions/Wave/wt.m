function varargout=wt(d,fmin,fmax,nbrSubOct,varargin)
...
Args=struct('Pad',1,...      % pad the time series with zeroes (recommended)
            ...
            'S0',968./fmax*0.001,...    % this says start at a scale of 2 years
            'J1',[],...
            'Mother','morlet', ... %'MORLET', 'PAUL', or 'DOG'
            'MaxScale',968./fmin*0.001,...   %a more simple way to specify J1
...
Yticks1 = 2.^(fix(log2(min(period))):fix(log2(max(period))));
...
varargout = {wave,period,scale,coi,sig95,sigma2};
