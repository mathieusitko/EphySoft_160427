function varargout=xwt(x,y,fmin,fmax,nbrSubOct,varargin)
...
Args=struct('Pad',1,...      % pad the time series with zeroes (recommended)
    'Dj',1/nbrSubOct, ...    % this will do 12 sub-octaves per octave
    'S0',968/fmax*dt,...    % this says start at a scale of 2 years
    'J1',[],...
    'Mother','Morlet', ...
    'MaxScale',968/fmin*dt,...   %a more simple way to specify J1
...
if Args.MakeFigure
    Yticks1 = 2.^((log2(min(period))):0.5:(log2(max(period))));
    %     Yticks2 = (myScal2frq(Yticks1,'morl',1/fech));
    Yticks2 = 1./Yticks1;
    ...
    %     set(HCB,'yticklabel',barylbls);
        ...
        set(gca,'YLim',log2([min(period),max(period)]), ...
            'YDir','reverse', ...
            'YTick',log2(Yticks1(:)), ...
            'YTickLabel',num2str(floor(Yticks2)'), ...
            'layer','top')
%            set(gca,        'YTick',1:2:length(1./period), ...
%             'YTickLabel',floor(10^1*(downsample(1./period,2)))*10^(-1), ...
%             'layer','top')  
