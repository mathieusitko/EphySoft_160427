function [ARF,RCF,PE,DC,ARB,PEB] = mvar(Y, Pmax, Mode)
...
elseif Mode==2 %%%%% Partial Correlation Estimation: Vieira-Morf Method [2] with unbiased covariance estimation
	%===== In [1] this algorithm is denoted "Nutall-Strand with unbiased covariance" =====%
        ...
        PEB = PE(:,1:M);                     % Eq 15.90
        for K = 1:Pmax,
                [D,n]	= covm(F(K+1:N,:),B(1:N-K,:),'M'); % Eq. 15.79
                D = D./n;

                ARF(:,K*M+(1-M:0)) = D / PEB;	% Eq 15.73 (Marple) - the minus sign is missing
                ARB(:,K*M+(1-M:0)) = D'/ PEF;	% Eq 15.74 (Marple) - the minus sign is missing
                
                tmp        = F(K+1:N,:) - B(1:N-K,:)*ARF(:,K*M+(1-M:0)).';  % Eq 15.84 - minus sign restored
                B(1:N-K,:) = B(1:N-K,:) - F(K+1:N,:)*ARB(:,K*M+(1-M:0)).';  % Eq 15.85 - minus sign restored
                F(K+1:N,:) = tmp;
                
                for L = 1:K-1,
                        tmp      = ARF(:,L*M+(1-M:0))   - ARF(:,K*M+(1-M:0))*ARB(:,(K-L)*M+(1-M:0)); % Eq 15.71 - minus sign restored
                        ARB(:,(K-L)*M+(1-M:0)) = ARB(:,(K-L)*M+(1-M:0)) - ARB(:,K*M+(1-M:0))*ARF(:,L*M+(1-M:0)); % Eq 15.72 - minus sign restored
                        ARF(:,L*M+(1-M:0))   = tmp;
                end;
                
                RCF(:,K*M+(1-M:0)) = ARF(:,K*M+(1-M:0));  % -Reflection coefficients (forward)
                RCB(:,K*M+(1-M:0)) = ARB(:,K*M+(1-M:0));  % -Reflection coefficients (backward)
                
                [PEF,n] = covm(F(K+1:N,:),F(K+1:N,:),'M'); % Eq 15.89
                PEF = PEF./n;

                [PEB,n] = covm(B(1:N-K,:),B(1:N-K,:),'M'); % Eq 15.89
                PEB = PEB./n;
