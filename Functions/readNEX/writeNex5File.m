...
%   writeContVarsAsFloats - if 1, data for continuous variables is written
%              as floating point numbers. This option preserves data
%              presision, but increases file size.
%
% OUTPUT:
%   nexFile - a structure containing .nex file data
...
if(isfield(nexFile, 'tend'))
    fwrite(fid, int64(nexFile.tend*nexFile.freq), 'int64');
else
    fwrite(fid, int64(0), 'int64');
end
...
for i = 1:contCount
    ...
    dataOffset = dataOffset + varHeader.Count*8 + varHeader.NumberOfDataPoints*dataPointSize;
end
